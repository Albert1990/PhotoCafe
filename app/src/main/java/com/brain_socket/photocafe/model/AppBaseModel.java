...
public abstract class AppBaseModel {
    ...
    public JSONObject getJsonObject(){
        try {
            PhotoCafeApp.getSharedGsonParser().toJson(this);
            return new JSONObject(PhotoCafeApp.getSharedGsonParser().toJson(this));
        }catch (Exception e){}
    ...
    public static <T> ArrayList<T> getArrayFromJsonSting(String jsonString, Type objectType) {
        try {
            ArrayList<T> object = PhotoCafeApp.getSharedGsonParser().fromJson(jsonString, objectType);
            return object;
        }catch (Exception e){
            e.printStackTrace();
        }
