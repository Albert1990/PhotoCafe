...
import com.brain_socket.photocafe.model.AppBaseModel;
...
public class DataCacheProvider {
    ...
    private DataCacheProvider() {
        try {
            // initialize
            prefData = PhotoCafeApp.appContext.getSharedPreferences("app_data", Context.MODE_PRIVATE);
    ...
    public void storeObjectWithKey(String key, Object arrayGroups) {
        try {
            if (arrayGroups != null) {
                String strJson = PhotoCafeApp.getSharedGsonParser().toJson(arrayGroups);
    ...
    public <T> T getStoredObjectWithKey(String key, Type objectType) {
        try {
            ...
            T object = PhotoCafeApp.getSharedGsonParser().fromJson(str, objectType);
            return object;
        }catch (Exception e){
            e.printStackTrace();
        }
    ...
    public <T> ArrayList<T> getStoredArrayWithKey(String key, Type objectType) {
        try {
            String str = prefData.getString(key, null);
            ArrayList<T> object = PhotoCafeApp.getSharedGsonParser().fromJson(str, objectType);
            return object;
        }catch (Exception e){
            e.printStackTrace();
        }
